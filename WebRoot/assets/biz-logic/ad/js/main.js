(function($){
	$(function(){
		...
		$('.menu-child').click(function(){
			...
			var location = '<a href="homePage.html" target="contentBody" onclick="home()">首页</a>';
		...
		$('#usermsg').click(function(){
			$('#location').html('用户信息');
			$('#contentBody').attr('src', 'user/user_message.html');
			$('.menu-child').removeClass('selected');
		});
		$('#logout').click(function(){
			asyncbox.confirm('确定要退出系统吗？','系统提示',function(action){ 
				if(action == 'ok'){
					document.location = window.rootPath+"/logout.html";
				}
			});
		});
	});
