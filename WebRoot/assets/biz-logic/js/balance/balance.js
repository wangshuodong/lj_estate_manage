...
$(function(){
	...
	$("input[name='pay_date']").val(formatDate(new Date(), "YYYY-MM-DD"));
	addApp_initialize();
	$('#detail_div').dialog({
		autoOpen: false,
		width: 650,
...
var formatter = {
	...
	certification : function(value,uid){
		if(value == 0)
			return "未上传";
		else if(value == 1)
			return "待审核";
		else if(value == 2)
			return "已认证<a href='./download.html?moduleid="+uid+"&filemodule=user'>下载</a>";
		else {
			return "";
		};
	},
...
{
	$.post('queryBalance.html',params,function(result){
		$("#daily_tbody").html('');
		...
		for(var i in dataList)
		{
			...
			var stylecss= "";
			if(dataObj.count == 0){
				 stylecss = '  style="background:yellow;cursor: pointer;"  title="第一次打款" ';
			}else if(dataObj.count == null){
				stylecss = '  style="background:yellow;cursor: pointer;"  title="第一次打款" ';
			}
			var tr_plan = '<tr>'
	  				+'<td align="center">'+index+'</td>'
	  				+'<td align="left" '+stylecss+' onclick="showInfo('+dataObj.userId+',\''+dataObj.username+'\')">用户名:'+dataObj.username+ '<br/>&nbsp;&nbsp;&nbsp;QQ:'
	  				+ dataObj.qq + '<br/>客服名称:' + dataObj.serviceName + '</td>'
	  				+'<td align="center">'+formatter.certification(dataObj.certification,dataObj.userId)+'</td>'
			...
			if (dataObj.status == 0) {
				tr_plan += '<a class="payment'+dataObj.balanceId+'" onClick="editDialog('+dataObj.balanceId+','
				+'\''+dataObj.username+'\''+','+'\''+dataObj.phone+'\''+','+'\''+dataObj.sdate+'\''+','+'\''+dataObj.edate+'\''+','+'\''+dataObj.bankname+'\''+','
				+'\''+dataObj.bankNo+'\''+','+'\''+dataObj.earnNoFee+'\''+');">付款</a><input type="checkbox" value="'+dataObj.balanceId+'" name="id"/>';
			}else if(dataObj.status == 2 ){
				
			} else if(dataObj.status == 1) {
				tr_plan += '<a onClick="detailDialog('+'\''+dataObj.province+'\''+','+'\''+dataObj.payUsername+'\''+','
				+'\''+dataObj.city+'\''+','+'\''+dataObj.bankname+'\''+','+'\''+dataObj.bankAddress+'\''+','+'\''+dataObj.bankUserName+'\''+','+'\''+dataObj.bankNo+'\''+','+'\''+dataObj.certificate+'\''+','
				+'\''+dataObj.payTime+'\''+','+'\''+dataObj.earnNoFee+'\''+');">详情</a>';
			}
...
function checkAll(o){
	if($(o).attr("checked") == null || $(o).attr("checked")=="undefined"){
...
function payClick(){
	...
	if(str == "" && oper_type=="payselected"){
		alert("你未选择任何用户");
		return false;
	}else if(str == "" && oper_type=="refuseSelected"){
		alert("你未选择任何用户");
		return false;
	}else{
		if(confirm("您确定需要继续执行该操作吗？请确认您的数据无误")){
			$.ajax({
				url:"batchPayBalance.html",
				type:"post",
				dataType:"json",
				beforeSend:function(){
					$("#loading").modal('show');
