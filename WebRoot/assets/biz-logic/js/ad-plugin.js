...
(function($){
    ...
    $.extend({
        ...
        _selectCascade : function(c_tag, parentCfg, isDefault){
            var js = '<script type="text/javascript" src="' + window.rootPath + "/cascade-" + parentCfg + ".js?v=" + new Date() + '"></script>';
            var head = $(document).find('head').append(js);
            
            function bindEvent(p_tag, c_tag, parentCfg, isDefault) {
                $(p_tag).bind('change', function() {
                    var cascade = $.fn.cascade, pKey = $(this).val();
                    if (c_tag.length && cascade && cascade[parentCfg]) {
                        c_tag.children().remove();
                        if (isDefault) {
                            c_tag.append('<option value="">==请选择==</option>');
                        }
                        var childs = cascade[parentCfg][pKey];
                        for (var i in childs) {
                            c_tag.append('<option value="' + i + '">' + childs[i] + '</option>');
                        }
                        c_tag._pullDownList('refresh');
                    }
                });
            }
        }*/
