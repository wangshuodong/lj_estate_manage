		...
		success:function(data){
			...
			if(data.list == null || data.list.length==0){
				...
				$.each(data.list,function(i,p){
					html += "<tr>";
					html += "<td>"+p.id+"</td>";
					html += "<td>"+p.houseName+"</td>";
					html += "<td>"+p.name+"</td>";
					html += "<td>"+p.remark+"</td>";
					html += "<td style='text-align:center;'>" +
							"<a href=\"javascript:;\" onclick=\"synchCommunityToAlipay("+p.id+")\"></a>&nbsp;&nbsp;&nbsp;&nbsp;" +
							"</td>";
...
function deleteBillAccount(id){
	if(window.confirm('你确定要删除当前楼栋账单吗？')){
...
function jumpToAdd(){
	window.location.href="addBuilding.html";
}
